...
def application(environ, start_response):
    ...
    if path == '/' or path == '':
        ...
        debug_info = f"FILE={index_file},EXISTS={os.path.exists(index_file)},SIZE="
        if os.path.exists(index_file):
            with open(index_file, 'r', encoding='utf-8') as f:
                content = f.read()
            debug_info += str(len(content))
            status = '200 OK'
            content_type = 'text/html; charset=utf-8'
        else:
            content = '<html><body><h1>index.html not found</h1><p>' + debug_info + '</p></body></html>'
